...
export default function WorkTeamTab({
  ...
  // Admin Direct Contributor Management State
  ...
  const handleAddContribSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!addContribName.trim() && !addContribUserId) return alert("لطفا نام همکار یا کاربر را مشخص نمایید.");
    try {
  ...
  const handleUpdateRole = async (userId: string, newRole: string) => {
    try {
      await apiClient.approveContributor(series.id, userId, 'update_role', user?.uid, newRole);
      alert("نقش همکار با موفقیت بروزرسانی شد.");
      onUpdateSeries();
    } catch (err: any) {
      alert("خطا در بروزرسانی نقش: " + err.message);
    }
  };

  const handleRemoveContrib = async (userId: string, displayName: string) => {
    if (!confirm(`آیا از حذف ${displayName} از تیم دست‌اندرکاران این اثر اطمینان دارید؟`)) return;
    try {
      await apiClient.approveContributor(series.id, userId, 'remove', user?.uid);
      alert("همکار با موفقیت از تیم حذف گردید.");
      onUpdateSeries();
    } catch (err: any) {
      alert("خطا در حذف همکار: " + err.message);
    }
  };
    ...
    <div className="space-y-8 text-right" dir="rtl">
      ...
      <div className="bg-[var(--color-asura-card)] border border-[var(--color-asura-border)] rounded-2xl p-6 shadow-lg">
        ...
        {approvedContributors.length === 0 ? (
          <p className="text-xs text-zinc-500">هنوز دست‌اندرکار تاییدشده‌ای برای این اثر ثبت نشده است.</p>
        ) : (
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3">
            {approvedContributors.map((c: any) => (
              <div key={c.userId} className="p-3 bg-black/40 border border-white/5 rounded-xl flex items-center justify-between gap-2">
                <div className="flex-1 min-w-0">
                  <span className="text-xs font-black text-white block truncate">{c.displayName}</span>
                  <span className="text-[10px] text-amber-400 font-bold block mt-0.5">
                    {c.role === "translator" ? "مترجم" : c.role === "cleaner" ? "کلینر" : c.role === "editor" ? "ادیتور" : c.role === "typesetter" ? "تایپیست" : c.role === "proofreader" ? "ویراستار" : c.role}
                  </span>
                  {c.email && <span className="text-[9px] text-zinc-500 font-mono block dir-ltr text-right truncate">{c.email}</span>}
                </div>
                ...
                {isGlobalAdmin ? (
                  <div className="flex items-center gap-1">
                    ...
                    <button
                      onClick={() => handleRemoveContrib(c.userId, c.displayName)}
                      title="حذف از پروژه"
                  ...
                  <span className="text-[9px] font-mono text-zinc-400 bg-white/5 px-2 py-1 rounded">
                    {c.melliCode || "عضو تیم"}
                  </span>
                )}
              </div>
        ...
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm flex items-center justify-center p-4 z-50">
          <div className="bg-[var(--color-asura-card)] border border-[var(--color-asura-border)] max-w-md w-full rounded-2xl p-6 text-right space-y-4">
            <h3 className="text-base font-black text-white flex items-center gap-2">
              ...
              افزودن مستقیم همکار به تیم
            </h3>
            <form onSubmit={handleAddContribSubmit} className="space-y-3">
              {staffList.length > 0 && (
                <div>
                  <label className="block text-xs font-bold text-zinc-300 mb-1">انتخاب از کادر اعضای تیم/سایت:</label>
                  <select
              ...
              <div>
                <label className="block text-xs font-bold text-zinc-300 mb-1">نام یا نام مستعار همکار *</label>
                <input
                  type="text"
                  required
              ...
              <div>
                <label className="block text-xs font-bold text-zinc-300 mb-1">نقش همکار در این اثر *</label>
                <select
              ...
              <div>
                <label className="block text-xs font-bold text-zinc-300 mb-1">ایمیل همکار (اختیاری)</label>
                <input
                  type="email"
